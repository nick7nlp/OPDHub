\section{White-Box On-Policy Methods: 密集信号与分布范式的转变}
\label{sec:white_box}
...
\subsubsection{ToDi (Token-wise Dynamic Divergence) \cite{todi2025}}
...
\subsubsection{AKL 与 Delta KD \cite{akl2024, deltakd2025}}
...
\subsubsection{Entropy-Aware OPD \cite{entropyopd2026}}
...
\subsubsection{DSKD (Dual Space Knowledge Distillation) \cite{dskd2025}}
...
\subsubsection{MiniLLM 与 Reverse KL 的详细推导 \cite{gu2023minillm}}
...
\textbf{理论差异对比：} Token-level 致力于最小化局部分布的差异（容易陷入 Greedy trap），而 Sequence-level 显式地建模了轨迹级别的 Reward。然而，由于依赖 REINFORCE 估计器，Sequence-level 方法面临极高的方差问题，往往需要复杂的 Baseline 控制（如 Constrained Optimization 视角 \cite{constrainedopd2025}）。
...
\begin{itemize}
    \item \textbf{PACED (Distillation at the Frontier of Student Competence) \cite{paced2026}:} 传统的 OPD 在两端浪费了大量计算资源：一方面是学生已经掌握的知识（梯度趋近于零），另一方面是远超学生能力的知识（产生破坏已有能力的噪声梯度）。PACED 提出了一种 Curriculum OPD 框架，精准识别并只在“学生能力边界”进行采样与蒸馏，使得蒸馏计算效率提升了 300\%。
    \item \textbf{Fast OPD from Reasoning Prefixes \cite{fastopd2026} \& PromptKD \cite{promptkd2024}:} 通过提供中间推理状态或结构化 Prompt，人为缩短了序列级探索的长度，有效平衡了 Token 密集监督与 Sequence 全局视野。
    \item \textbf{$\mathcal{X}$-KD (Experiential KD) \cite{xkd2026}:} 引入了“体验式”学习框架，允许模型在其历史错误轨迹上进行 Counterfactual (反事实) 的 On-policy 蒸馏。
\end{itemize}

\subsection{Theoretical Analysis \& Method Comparison}
\label{subsec:theory_whitebox}

在理论层面，\cite{rethinkingkl2024} 深入剖析了 Forward KL 与 Reverse KL 的拓扑差异。在 On-policy 范式下，Forward KL 是 \textbf{Mean-seeking} 的，倾向于覆盖教师模型的所有模式（即使在此过程中产生零概率密度的幻觉）；而 Reverse KL 是 \textbf{Mode-seeking} 的，它通过牺牲多样性来确保生成的轨迹严格处于教师的高置信度区域。统一的 $f$-divergence 框架通过调整生成函数的凸性，实现了在 Mode-seeking 与 Mean-seeking 之间的平滑过渡。
...
\section{Black-Box and Self-Distillation Methods: 稀疏反馈与自我迭代}
\label{sec:black_box}
...
\begin{itemize}
    \item \textbf{Zephyr \cite{zephyr2023}:} 开创性地将直接偏好优化 (DPO) 引入蒸馏过程 (Direct Distillation from Preference, dDPO)。通过将强大黑盒模型（如 GPT-4）生成的回答作为 $y_{win}$，学生自生成的回答作为 $y_{lose}$，构建 On-policy 偏好对，直接跨越了 Logit 映射的障碍。
    ...
    \item \textbf{ORPO-Distill \cite{orpo2025}:} 将 Odds Ratio Preference Optimization 与黑盒蒸馏结合，在无需显式 Reward Model 的前提下，通过惩罚学生模型自生成的低质 Token 序列，实现了高效的黑盒知识转移。
\end{itemize}
...
\subsubsection{SPIN (Self-Play Fine-Tuning) \cite{spin2024}}
...
\begin{itemize}
    \item \textbf{OPSD (On-Policy Self-Distillation) \cite{opsd2026}:} 提出在不同温度参数（Temperature）下的自我分布之间进行差异化学习。利用高温分布进行泛化探索，低温分布进行置信度锚定。
    \item \textbf{Multi-Token Prediction Self-Distillation (MTP) \cite{mtp2026}:} 创新性地将自我蒸馏应用于多步预测加速。模型无需额外的 Speculator，即可通过 On-policy 轨迹将未来的隐式状态蒸馏到当前的表征中。
    \item \textbf{On-Policy Context Distillation (OPCD) \cite{opcd2026}:} 将 Prompt 中包含的 In-context 知识，通过自生成的 Rollout 轨迹内化到模型权重中。在此过程中，模型自己充当过滤掉冗余 Context 的“提纯器”。
\end{itemize}

\subsection{The Convergence of Self-Play (深度分析：饱和与突破)}
...
\textbf{突破饱和的策略：} 
最近的研究（如 Online Experiential Learning \cite{onlineexp2026}）表明，要打破这种自我博弈的死锁，必须引入两类破局机制：
\begin{enumerate}
    \item \textbf{知识注入的非对称性：} 强制在 Self-play 循环中引入具有高度随机性的外部环境反馈（例如调用代码执行器、定理证明器等物理级/逻辑级验证器），提供硬性稀疏奖励，这被称为 Agent-Level OPD。
    \item \textbf{Distillation-RL Loop 融合：} 将白盒蒸馏的高保真度与黑盒强化的探索性深度耦合。未来的 Scaling Laws 不再仅仅依赖于预训练的数据量，而是依赖于\textbf{推理阶段（Test-Time Compute）} 的计算量扩展（如近期爆火的测试时缩放研究 \cite{testtime2025}）。在此背景下，Distillation 将从一种“模型压缩技术”演化为“Test-time 搜索轨迹的高效缓存与提炼技术”。
\end{enumerate}
