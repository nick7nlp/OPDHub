...
\begin{enumerate}
    ...
    \item \textbf{Actionable Guidance:} We provide a concrete practitioner decision tree (Section~\ref{subsec:decision_tree}) that maps deployment constraints to recommended methods, compute budget guidelines, and a ``When to Use On-Policy vs.\ Off-Policy'' reference (Section~\ref{subsec:when_onpolicy}).
\end{enumerate}
...
\textbf{Temporally adaptive targets.} TAID~\citep{2501.16937} constructs a time-dependent intermediate distribution $P_t = (1-\lambda_t) P_\theta + \lambda_t P_T$ where $\lambda_t$ gradually increases during training, effectively starting with a target close to the student (easy to match) and progressively approaching the full teacher distribution. This temporal curriculum over the \emph{target itself} (rather than over the input prompts as in PACED) provides a complementary axis of control: PACED asks ``which problems to train on,'' while TAID asks ``how much of the teacher's knowledge to reveal at each stage.'' The combination of prompt-level curriculum (PACED) with target-level curriculum (TAID) represents the current frontier of training dynamics optimization.
...
\textbf{Behavioral PI: context distillation.} The PI paradigm extends beyond factual ground truth to behavioral instructions. OPCD~\citep{2602.12275} treats system prompts, exemplars, and retrieval context as privileged information, enabling a model to internalize behaviors that would otherwise require expensive in-context demonstrations at inference time. OPSDL~\citep{2604.17535} extends this to long-context settings: using the model's short-context capability as a privileged teacher that supervises on-policy generation over extended sequences, enabling knowledge transfer from the model's well-calibrated short-context behavior to its degraded long-context behavior without requiring any external teacher. OEL~\citep{2603.16856} extends to continuous post-deployment learning, using interaction outcomes as dynamic PI that updates the model's teacher policy as it accumulates experience, closing a loop that OPCD opened.
...
Self-Distilled RLVR~\citep{yang2026selfdistilled} combines self-distillation with verification-based RL, showing that the two mechanisms are complementary: self-distillation provides the dense signal for stable training while RLVR provides the sparse but accurate outcome signal for quality improvement.
...
\textbf{Self-adjusting curricula.} $\pi$-Play's multi-agent framework (Section~\ref{subsec:self_play}) provides an alternative curriculum mechanism: as the student strengthens, the examiner agent automatically generates harder tasks, creating a self-adjusting difficulty ladder that requires no external difficulty estimation. PRISM~\citep{li2026prism} provides a more structured approach: progressive reasoning via iterative self-distillation with a mastery-based curriculum that only advances the student to harder problems once it demonstrates consistent performance on the current difficulty tier. PAINT~\citep{wang2026paint} addresses the prefix-alignment problem in instruction tuning, ensuring that the student's on-policy rollouts are conditioned on properly formatted instruction prefixes that match the deployment format, preventing a subtle form of distribution mismatch between training and inference.
...
\textbf{The calibration-capability gap.} CaOPD~\citep{2604.16830} reveals that on-policy distillation can improve task accuracy while simultaneously degrading model calibration: the student becomes more capable but less aware of its own uncertainty boundaries. By replacing self-reported confidence (which distilled models systematically overestimate) with empirical confidence derived from $K$ rollouts, CaOPD decouples capability improvement from calibration preservation. This finding has important implications for deployment: a distilled model that achieves higher benchmark scores but cannot reliably indicate when it might be wrong is arguably less safe than a weaker but better-calibrated baseline. The ``illusion of certainty'' connects to the saturation analysis above: both indicate that OPD can produce students that lose metacognitive capabilities (uncertainty awareness, epistemic verbalization) even as task-level performance improves.
...
\begin{table*}[t]
    ...
    \begin{tabular}{@{}lclllll@{}}
        ...
        IRIS \citep{2604.20933} & 2026 & Self-Play & Interpolative R\'{e}nyi self-play & Self & Sequence & Unifies SPIN/SPIF/DPO \\
        ...
        PRISM \citep{li2026prism} & 2026 & Self (SP) & Mastery-based curriculum & Self & Sequence & Progressive difficulty self-distill \\
        \midrule
