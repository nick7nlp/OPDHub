...
\begin{equation}
    \loss_{\text{ToDi}} = \E_{y \sim \ptheta} \left[ \sum_{t=1}^{|y|} w_t \cdot \KL(\ptheta \parallel \pteacher) + (1-w_t) \cdot \KL(\pteacher \parallel \ptheta) \right]
\end{equation}
where $w_t$ emphasizes Reverse KL for tokens the student overestimates and Forward KL for tokens it underestimates. Entropy-Aware OPD~\citep{2603.07079} takes an alternative approach, using the teacher's entropy $H(\pteacher(\cdot|y_{<t}))$ as the interpolation signal:
\begin{equation}
