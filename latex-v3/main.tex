...
\begin{tikzpicture}[%
  ...
  level 1/.style={sibling distance=60mm, level distance=18mm},
  level 2/.style={sibling distance=28mm, level distance=16mm},
  root/.style={draw=deepblue, fill=lightblue2!60, rounded corners=5pt, line width=1.2pt, font=\bfseries\small, inner sep=6pt, minimum width=50mm},
  branch/.style={draw=midblue, fill=lightblue!40, rounded corners=4pt, line width=0.8pt, font=\bfseries\footnotesize, inner sep=5pt, minimum width=38mm},
  leaf/.style={draw=midblue!50, fill=white, rounded corners=3pt, line width=0.5pt, font=\scriptsize, inner sep=3pt, text width=26mm, align=center},
  edge from parent/.style={draw=midblue!70, line width=0.8pt, -{Stealth[length=4pt]}}
]
\node[root] {On-Policy Distillation}
  child {node[branch] {\S\ref{sec:objectives} Objective}
    child {node[leaf] {Fixed Divergence\\{\tiny GKD, MiniLLM, DistiLLM}}}
    child {node[leaf] {Adaptive Divergence\\{\tiny ToDi, AKL, Entropy-Aware}}}
    child {node[leaf] {RL-Augmented\\{\tiny G-OPD, KDRL, RLAD}}}
  }
  child {node[branch] {\S\ref{sec:signal} Signal Source}
    child {node[leaf] {White-Box\\{\tiny DSKD, Delta-KD, Cross-Tok, TAID}}}
    child {node[leaf] {Black-Box / API\\{\tiny GAD, Lion, OVD, DAIL}}}
    child {node[leaf] {Self-Distillation\\{\tiny OPSD, SPIN, SD-ZERO, $\pi$-Play}}}
  }
  child {node[branch] {\S\ref{sec:dynamics} Dynamics}
    child {node[leaf] {Token/Sample Weight\\{\tiny TIP, SCOPE, SelecTKD}}}
    child {node[leaf] {Curriculum\\{\tiny PACED, AdaSwitch}}}
    child {node[leaf] {Compute Efficiency\\{\tiny Fast OPD, Lightning, Spec.KD}}}
  };
\end{tikzpicture}
}%
\caption{Taxonomy of On-Policy Distillation methods organized by the practitioner's decision chain. Each method is placed in the category corresponding to its \emph{core contribution}. The three stages are sequential: first choose the optimization objective, then identify the signal source, then address training dynamics.}
\label{fig:taxonomy}
\end{figure*}
