...
\textbf{MiniLLM} \citep{2306.08543}\textbf{.} MiniLLM identifies that Forward KL forces over-allocation to the teacher's long tail. To enforce mode-seeking behavior, it selects Reverse KL ($\mathcal{D}_f = \KL(\ptheta \parallel \pteacher)$). For sampling, MiniLLM generates trajectories purely from the student policy, setting $\pi_{\mathrm{mix}} = \ptheta$, so that the student confronts its own errors during training. Because Reverse KL places the student parameters in both the expectation and the log-ratio, MiniLLM reformulates optimization via the Policy Gradient theorem (REINFORCE), treating the teacher's log-probability as a reward. Defining the cumulative reward-to-go $R_t = \sum_{t'=t}^{|y|} \log \frac{\pteacher(y_{t'}|y_{<t'})}{\ptheta(y_{t'}|y_{<t'})}$, the gradient becomes
\begin{equation}
...
\textbf{Logit-Based Feedback.} The student's generated sequence is evaluated against the teacher's continuous probability distribution. Methods like GKD \citep{2306.13649} compute token-level $f$-divergences at every generative step, while MiniLLM \citep{2306.08543} uses the teacher's per-token log-probabilities to construct a sequence-level reward optimized via policy gradients. More recent work, such as ToDi \citep{2505.16297}, adaptively combines Forward and Reverse KL per token using a sigmoid weight derived from the teacher-student log-probability ratio. Logit-based feedback provides the densest gradient signal, as the loss vector has size $|V|$ at every step $t$, ensuring the student captures exact teacher decision boundaries.
...
\begin{table*}[t]
    ...
    \begin{tabular}{@{}lclllll@{}}
        ...
        DistiLLM \citep{2402.03898}       & 2024 & Token  & Skew KL (SKL+SRKL)  & White-box & Off-Policy (Adaptable)  & $\alpha$-smoothing for KL \\
...
The distillation step is straightforward. 600,000 long-CoT reasoning samples are generated from DeepSeek-R1 \emph{offline}, combined with 200,000 non-reasoning samples, and smaller student models (Qwen-1.5B through Llama-70B) are fine-tuned on this 800K dataset via standard supervised fine-tuning (SFT). No on-policy student rollouts, no logit matching, no iterative teacher-student interaction. The student never generates its own responses during training. This is classical Sequence-Level Knowledge Distillation \citep{kim2016sequence} applied at large scale.
